...
若经过 Nginx 或其他反向代理，流式响应可能被缓冲，导致前端不是「逐字」而是「整段」才出来。需要在 Nginx 里对该接口关缓冲，例如：
...
项目在 RAG 之上做了多级缓存（L1 精确 + L2 语义），减少重复问对 LLM 的调用。注意：

- 语义缓存一般用「问题向量」的余弦相似度做命中判断，阈值（如 0.95）要自己根据业务调；太高几乎不命中，太低容易误命中。
- 若文档或分块策略有变更，要考虑是否清空或失效相关缓存（项目里有按知识库清缓存的接口）。
...
| 流式     | 代理缓冲导致不逐字                     | Nginx 关 buffering                                |
